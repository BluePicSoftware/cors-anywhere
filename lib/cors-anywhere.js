...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var hostnameWhiteList = [/^.*\.googleusercontent\.com$/];

    var relatedHostname = hostnameWhiteList.find(regex => regex.test(location.hostname));
    console.log(relatedHostname);

    if (relatedHostname === undefined) {
      res.writeHead(400);
      res.end('The hostname is not on the whitelist');
      return;
    }
    //  && hostnameWhiteList.find(regex => regex.test(hostname));
    ...
    var origin = req.headers.origin || process.env.DEFAULT_ORIGIN;
